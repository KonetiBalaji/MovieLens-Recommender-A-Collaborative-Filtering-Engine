...
def run_step_3():
    ...
    rmse_score = evaluate_model(df, user_item_matrix, similarity_df, k=5)
    print(f"RMSE on sample of 500 ratings: {rmse_score:.4f}")

    output_path = "outputs/user_cf_recommendations.csv"
...
def run_item_cf_step():
    ...
    print(f"\n[Item-Based CF] Predicted rating for user {test_user} on item {test_item}: {predicted_rating:.2f}")
    ...
    print("[Item-Based CF] Recommendations saved to 'outputs/item_cf_recommendations.csv'")
...
def run_matrix_factorization_step():
    ...
    top_mf_recs = recommend_top_n_from_mf(
        user_id=0,
...
def enrich_user_cf_recommendations():
    print("\n[Enrichment] Loading metadata and enriching user-based recommendations...")
    metadata = load_movie_metadata()
    recommendations = pd.read_csv("outputs/user_cf_recommendations.csv")
    enriched = enrich_recommendations(recommendations, metadata)

    print("\n[Enrichment] Sample enriched recommendations:")
    print(enriched.head())

    enriched.to_csv("outputs/user_cf_enriched.csv", index=False)
    print("[Enrichment] Saved to 'outputs/user_cf_enriched.csv'")
...
if __name__ == "__main__":
    ...
    enrich_user_cf_recommendations()
